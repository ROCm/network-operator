...
type upgradeMgrAPI interface {
	HandleUpgrade(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, nodes *v1.NodeList) (ctrl.Result, error)
	HandleDelete(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, nodes *v1.NodeList) (ctrl.Result, error)
...
func newUpgradeMgrHandler(client client.Client, k8sConfig *rest.Config, isOpenShift bool, workerMgr workermgr.WorkerMgrAPI) upgradeMgrAPI {
	k8sIntf, err := kubernetes.NewForConfig(k8sConfig)
	if err != nil {
		return nil
	}
	return &upgradeMgr{
		helper: newUpgradeMgrHelperHandler(client, k8sIntf, isOpenShift, workerMgr),
	}
}

/*================================= Upgrade Manager APIs===================================*/

// HandleUpgrade handles the upgrade functionalities for networkconfig
func (n *upgradeMgr) HandleUpgrade(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, nodeList *v1.NodeList) (ctrl.Result, error) {
	res := ctrl.Result{}

	var candidateNodes []v1.Node
	var upgradeDone, upgradeInProgress, upgradeFailedState, installInProgress int

	// if driver section is disabled in network config, skip upgrade policy handler
	if networkConfig.Spec.Driver.Enable == nil || !*networkConfig.Spec.Driver.Enable {
		return ctrl.Result{}, nil
	}
	if networkConfig.Spec.Driver.UpgradePolicy == nil ||
		(networkConfig.Spec.Driver.UpgradePolicy.Enable != nil &&
			!*networkConfig.Spec.Driver.UpgradePolicy.Enable) {
	...
	initInternalNodeStates := func(networkConfig *amdv1alpha1.NetworkConfig) {
		for nodeName, moduleStatus := range networkConfig.Status.NodeModuleStatus {
			if moduleStatus.Status == amdv1alpha1.UpgradeStateStarted {
				if networkConfig.Spec.Driver.UpgradePolicy.RebootRequired != nil && *networkConfig.Spec.Driver.UpgradePolicy.RebootRequired {
					...
					if err == nil {
						...
						if nodeObj.Status.NodeInfo.BootID == moduleStatus.BootId {
							...
							n.helper.handleNodeReboot(ctx, nodeObj, *networkConfig)
					...
					go n.helper.deleteRebootPod(ctx, nodeName, *networkConfig, true)
				...
				rebootPod := n.helper.getRebootPod(nodeName, networkConfig)
				podObj, err := n.helper.getPod(ctx, rebootPod.Name, networkConfig.Namespace)
				if err != nil {
	...
	if n.helper.isInit() {
		...
		initInternalNodeStates(networkConfig)
	}

	if n.helper.specChanged(networkConfig) {
		/* Reset internal states */
		n.helper.clearNodeStatus()
	}
	n.helper.setcurrentSpec(networkConfig)

	for i := 0; i < len(nodeList.Items); i++ {

		// 1. Set init status for unprocessed nodes
		n.helper.handleInitStatus(ctx, &nodeList.Items[i], networkConfig)

		// 2. Handle an upgrade going on for very long
		n.helper.handleUpgradeTimedOut(ctx, &nodeList.Items[i], networkConfig)
		...
		if n.helper.isNodeNmcStatusMissing(ctx, &nodeList.Items[i], networkConfig) {
			upgradeInProgress++
			continue
		}
		...
		if n.helper.isNodeReady(ctx, &nodeList.Items[i], networkConfig) {
			n.helper.clearUpgradeStartTime(nodeList.Items[i].Name)
			upgradeDone++
			continue
		}
		...
		// 7. Handle New nodes
		if n.helper.isNodeNew(ctx, &nodeList.Items[i], networkConfig) {
			// Driver will be unconditionally installed on new node
			installInProgress++
			continue
		}

		// 8. Handle Driver Install In Progress nodes
		if n.helper.isNodeStateInstallInProgress(ctx, &nodeList.Items[i], networkConfig) {
			installInProgress++
			continue
		}

		// 9. Handle Driver Upgrade In Progress nodes
		if n.helper.isNodeStateUpgradeInProgress(ctx, &nodeList.Items[i], networkConfig) {
			upgradeInProgress++
			continue
		}
	...
	maxParallelUpgrades, policyViolated := n.helper.isUpgradePolicyViolated(upgradeInProgress, upgradeFailedState, len(nodeList.Items), networkConfig)
	...
	for i := 0; i < (maxParallelUpgrades-upgradeInProgress) && i < len(candidateNodes); i++ {
		...
		go n.helper.handleNodeUpgrade(ctx, *networkConfig, candidateNodes[i])
...
func (n *upgradeMgr) HandleDelete(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, nodeList *v1.NodeList) (res ctrl.Result, err error) {

	for i := 0; i < len(nodeList.Items); i++ {
		if err := n.helper.cordonOrUncordonNode(ctx, networkConfig, &nodeList.Items[i], false); err != nil {
...
type upgradeMgrHelperAPI interface {
	...
	handleInitStatus(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig)

	// Handle node state transitions
	isNodeReady(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool
	isNodeNmcStatusMissing(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool
	isNodeNew(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool
	isNodeStateUpgradeStarted(node *v1.Node) bool
	isNodeStateInstallInProgress(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool
	isNodeStateUpgradeInProgress(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool
	...
	isUpgradePolicyViolated(upgradeInProgress int, upgradeFailedState int, totalNodes int, networkConfig *amdv1alpha1.NetworkConfig) (int, bool)

	// Helper APIs for upgrade-in-progress nodes
	cordonOrUncordonNode(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node, add bool) error
	handleNodeUpgrade(ctx context.Context, networkConfig amdv1alpha1.NetworkConfig, node v1.Node)
	isNetworkConfigValid(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig) bool
	getPodsToDrainOrDelete(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) (newPods []v1.Pod, err error)
	...
	handleNodeReboot(ctx context.Context, node *v1.Node, nc amdv1alpha1.NetworkConfig)
	deleteRebootPod(ctx context.Context, nodeName string, nc amdv1alpha1.NetworkConfig, force bool)
	getRebootPod(nodeName string, nc *amdv1alpha1.NetworkConfig) *v1.Pod
	hasUpgradeTimeExceeded(ctx context.Context, nodeName string, networkConfig *amdv1alpha1.NetworkConfig) bool
	handleUpgradeTimedOut(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig)

	// getters and setters
	specChanged(networkConfig *amdv1alpha1.NetworkConfig) bool
	setcurrentSpec(networkConfig *amdv1alpha1.NetworkConfig)
	getNodeStatus(nodeName string) amdv1alpha1.UpgradeState
...
func newUpgradeMgrHelperHandler(client client.Client, k8sInterface kubernetes.Interface, isOpenShift bool, workerMgr workermgr.WorkerMgrAPI) upgradeMgrHelperAPI {
...
func (h *upgradeMgrHelper) handleInitStatus(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) {
	...
	if h.isNodeInFailedUpgradeStates(nodeStatus) {
		...
		if h.isLabelUpgradeRequiredOnNode(ctx, networkConfig, node) {
			...
			if err := h.resetModuleVersionOnNode(ctx, networkConfig, node); err == nil {
			...
			if err := h.cleanupDanglingKMMPods(ctx, node, networkConfig); err != nil {
				return
			}
...
func (h *upgradeMgrHelper) cleanupDanglingKMMPods(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) error {
	// Get Worker Pods
	workerLabelSelector := labels.SelectorFromSet(map[string]string{
		"app.kubernetes.io/component":        "worker",
		"app.kubernetes.io/part-of":          "kmm",
		"kmm.node.kubernetes.io/module.name": networkConfig.Name,
	})
...
func (h *upgradeMgrHelper) isNodeNew(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool {
	...
	if moduleName, ok := node.Labels[fmt.Sprintf("kmm.node.kubernetes.io/version-module.%s.%s", networkConfig.Namespace, networkConfig.Name)]; !ok {
		...
		if nodeStatus, ok := networkConfig.Status.NodeModuleStatus[node.Name]; ok {
			if moduleName != networkConfig.Spec.Driver.Version && nodeStatus.Status == amdv1alpha1.UpgradeStateInstallInProgress {

				// Update expected module version on the node
				if err := h.updateModuleVersionOnNode(ctx, networkConfig, node); err != nil {
...
func (h *upgradeMgrHelper) isNodeNmcStatusMissing(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool {
	...
	if nodeStatus, ok := networkConfig.Status.NodeModuleStatus[node.Name]; ok {
		...
		if nodeStatus.ContainerImage == "" && currentState == amdv1alpha1.UpgradeStateInProgress {
			...
			// Uncordon the node
			if err := h.cordonOrUncordonNode(ctx, networkConfig, node, false); err != nil {
...
func (h *upgradeMgrHelper) isNodeReady(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool {
	...
	if nodeStatus, ok := networkConfig.Status.NodeModuleStatus[node.Name]; ok {
		...
		driverVersion, _ := utils.GetDriverVersion(*node, *networkConfig)
		...
		if strings.HasSuffix(nodeStatus.ContainerImage, driverVersion) {
			...
			// load back amdgpu
			// if upgrademgr is installing on new node, no need to load amdgpu again as it would have been loaded as part of initial install
			if currentState != amdv1alpha1.UpgradeStateInstallInProgress {
				if err := h.loadAMDGPUDriver(ctx, networkConfig, node); err != nil {
					log.FromContext(ctx).Error(err, fmt.Sprintf("Node: %v. Failed to do AMDGPU driver load with error: %v", node.Name, err))
					// Move to failure state if loading amdgpu fails
					h.setNodeStatus(ctx, node.Name, amdv1alpha1.UpgradeStateFailed)
					return false
				}
			}

			// remove the worker pod state node label
			if err := h.removeWorkerPodNodeLabel(ctx, networkConfig, node); err != nil {
				log.FromContext(ctx).Error(err, fmt.Sprintf("Node: %v. Failed to remove worker pod node label with error: %v", node.Name, err))
				// Move to failure state if removing worker pod node label fails
				h.setNodeStatus(ctx, node.Name, amdv1alpha1.UpgradeStateFailed)
				return false
			}

			// Uncordon the node
			// if amdgpu related kernel modules are managed by KMM
			// uncordon the node will auto bring them back
			if err := h.cordonOrUncordonNode(ctx, networkConfig, node, false); err != nil {
				// Move to failure state if uncordon fails
				h.setNodeStatus(ctx, node.Name, amdv1alpha1.UpgradeStateUncordonFailed)
				return false
...
func (h *upgradeMgrHelper) isNodeStateInstallInProgress(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool {
...
func (h *upgradeMgrHelper) isNodeStateUpgradeInProgress(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) bool {

	// CR update when network operator restarted.
	currentStatus := h.getNodeStatus(node.Name)
	if moduleName, ok := node.Labels[fmt.Sprintf("kmm.node.kubernetes.io/version-module.%s.%s", networkConfig.Namespace, networkConfig.Name)]; ok {
		if moduleName != networkConfig.Spec.Driver.Version && currentStatus == amdv1alpha1.UpgradeStateInProgress {
...
func (h *upgradeMgrHelper) isUpgradePolicyViolated(upgradeInProgress int, upgradeFailedState int, totalNodes int, networkConfig *amdv1alpha1.NetworkConfig) (int, bool) {

	maxParallelUpdates := networkConfig.Spec.Driver.UpgradePolicy.MaxParallelUpgrades
	maxUnavailableNodes, err := intstr.GetScaledValueFromIntOrPercent(&networkConfig.Spec.Driver.UpgradePolicy.MaxUnavailableNodes, totalNodes, true)
	if err != nil {
...
func (h *upgradeMgrHelper) handleUpgradeTimedOut(ctx context.Context, node *v1.Node, networkConfig *amdv1alpha1.NetworkConfig) {

	nodeStatus := h.getNodeStatus(node.Name)
	if nodeStatus == amdv1alpha1.UpgradeStateStarted ||
		nodeStatus == amdv1alpha1.UpgradeStateInProgress ||
		nodeStatus == amdv1alpha1.UpgradeStateRebootInProgress {
		if h.hasUpgradeTimeExceeded(ctx, node.Name, networkConfig) {
...
func (h *upgradeMgrHelper) specChanged(networkConfig *amdv1alpha1.NetworkConfig) bool {
	if h.currentSpec.version != "" && h.currentSpec.version != networkConfig.Spec.Driver.Version {
		return true
	}
	return false
}

func (h *upgradeMgrHelper) setcurrentSpec(networkConfig *amdv1alpha1.NetworkConfig) {
	h.currentSpec.version = networkConfig.Spec.Driver.Version
	h.currentSpec.enable = false
	if networkConfig.Spec.Driver.Enable != nil {
		h.currentSpec.enable = *networkConfig.Spec.Driver.Enable
	}
}

func (h *upgradeMgrHelper) deleteOrDrainPods(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) error {

	pods, err := h.getPodsToDrainOrDelete(ctx, networkConfig, node)
	...
	if len(pods) > 0 {
		...
		nc := networkConfig.Spec.Driver.UpgradePolicy
		if nc.NodeDrainPolicy != nil {
			h.drainHelper.Force = false
			if nc.NodeDrainPolicy.Force != nil && *nc.NodeDrainPolicy.Force {
				h.drainHelper.Force = *nc.NodeDrainPolicy.Force
			}
			h.drainHelper.Timeout = time.Duration(float64(nc.NodeDrainPolicy.TimeoutSeconds) * float64(time.Second))
			h.drainHelper.GracePeriodSeconds = nc.NodeDrainPolicy.GracePeriodSeconds
		} else if nc.PodDeletionPolicy != nil {
			h.drainHelper.DisableEviction = true
			h.drainHelper.Force = false
			if nc.PodDeletionPolicy.Force != nil && *nc.PodDeletionPolicy.Force {
				h.drainHelper.Force = *nc.PodDeletionPolicy.Force
			}
			h.drainHelper.Timeout = time.Duration(float64(nc.PodDeletionPolicy.TimeoutSeconds) * float64(time.Second))
			h.drainHelper.GracePeriodSeconds = nc.PodDeletionPolicy.GracePeriodSeconds
		}
...
func (h *upgradeMgrHelper) getPodsToDrainOrDelete(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) (newPods []v1.Pod, err error) {
	...
	for _, pod := range pods.Items {
		if strings.HasPrefix(pod.Name, fmt.Sprintf("%v-%v", networkConfig.Name, "metrics-exporter")) ||
			strings.HasPrefix(pod.Name, fmt.Sprintf("%v-%v", networkConfig.Name, "device-config-manager")) ||
			strings.HasPrefix(pod.Name, fmt.Sprintf("%v-%v", networkConfig.Name, "device-plugin")) ||
			strings.HasPrefix(pod.Name, fmt.Sprintf("%v-%v", networkConfig.Name, "node-labeller")) ||
			strings.HasPrefix(pod.Name, fmt.Sprintf("%v-%v", networkConfig.Name, "test-runner")) {
		...
		for _, container := range pod.Spec.Containers {
			...
			for _, volumeMount := range container.VolumeMounts {
				if strings.HasPrefix(volumeMount.MountPath, "/dev/dri") ||
					strings.HasPrefix(volumeMount.MountPath, "/dev/kfd") ||
					strings.HasPrefix(volumeMount.MountPath, "/dev") {
					newPods = append(newPods, pod)
					break podLoop
				}
			}
...
func (h *upgradeMgrHelper) handleNodeUpgrade(ctx context.Context, networkConfig amdv1alpha1.NetworkConfig, node v1.Node) {
	...
	if err := h.client.Get(ctx, client.ObjectKey{Name: node.Name}, nodeObj); err == nil {
		if version, ok := nodeObj.Labels[fmt.Sprintf("kmm.node.kubernetes.io/version-module.%s.%s", networkConfig.Namespace, networkConfig.Name)]; ok {
			if version == networkConfig.Spec.Driver.Version {
	...
	cordonErr := h.cordonOrUncordonNode(ctx, &networkConfig, &node, true)
	if networkConfigValid := h.isNetworkConfigValid(context.TODO(), &networkConfig); networkConfigValid {
		if cordonErr != nil {
		...
		// Proceed if the network config is valid and cordoning is successful
	} else {
		// network config changed when cordoning was going on. Dont proceed
		return
	}

	// Drain the pods that are using network driver
	drainErr := h.deleteOrDrainPods(ctx, &networkConfig, &node)
	if networkConfigValid := h.isNetworkConfigValid(context.TODO(), &networkConfig); networkConfigValid {
		if drainErr != nil {
		...
		// network config changed when draining was going on. Don't proceed
		return
	}

	// Reboot the node if required
	if networkConfig.Spec.Driver.UpgradePolicy.RebootRequired != nil && *networkConfig.Spec.Driver.UpgradePolicy.RebootRequired {
		h.handleNodeReboot(ctx, &node, networkConfig)
	} else {
		// unload AMDGPU driver before upgrading the network driver
		if h.shouldTriggerAMDGPUWorkerPod(ctx, &networkConfig, &node) {
			if err := h.unloadAMDGPUDriver(ctx, &networkConfig, &node); err != nil {
		...
		if err := h.waitForAMDGPUUnload(ctx, &networkConfig, &node); err != nil {
			logger.Error(err, fmt.Sprintf("Node: %v State: %v UpgradeFailed with Error: %v", node.Name, h.getNodeStatus(node.Name), err))
			// Mark the state as failed
			h.setNodeStatus(ctx, node.Name, amdv1alpha1.UpgradeStateFailed)
			return
...
func (h *upgradeMgrHelper) shouldTriggerAMDGPUWorkerPod(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) bool {
	logger := log.FromContext(ctx)
	label := h.workerMgr.GetWorkReadyLabel(types.NamespacedName{Namespace: networkConfig.Namespace, Name: networkConfig.Name})
	if bootID, exists := node.Labels[label]; exists {
		if bootID == node.Status.NodeInfo.BootID {
			// the amdgpu unload/load worker pod
			// has already been triggered for this boot
			// no need to trigger again
			logger.Info(fmt.Sprintf("Node: %v. AMDGPU unload worker pod has already been triggered for this boot %v", node.Name, bootID))
			return false
		}
		// if ready label exists but bootID does not match
		// it means node was rebooted, amdgpu may have been loaded back
		// need to trigger amdgpu unload/load worker pod again
		...
		if retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
			...
			logger.Error(retryErr, fmt.Sprintf("failed to remove work ready label from node %s", node.Name))
			return true
		}
		logger.Info(fmt.Sprintf("Node: %v. Node was rebooted, need to trigger AMDGPU unload worker pod again", node.Name))
		return true
	}
	// if label does not exist, it means amdgpu was not reloaded
	logger.Info(fmt.Sprintf("Node: %v. Need to trigger AMDGPU unload worker pod", node.Name))
	return true
}

func (h *upgradeMgrHelper) isKMMManagedAMDGPU(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) bool {
	// Check if node has gpu.operator.amd.com/<namespace>.<name>.driver label
	driverLabelPrefix := "gpu.operator.amd.com/"
	driverLabelSuffix := ".driver"
	for label := range node.Labels {
		if strings.HasPrefix(label, driverLabelPrefix) && strings.HasSuffix(label, driverLabelSuffix) {
			return true
		}
	}

	// Check if amdgpu module exists in NMC
	nmc := &kmmv1beta1.NodeModulesConfig{}
	if err := h.client.Get(ctx, client.ObjectKey{Name: node.Name}, nmc); err != nil {
		return false
	}
	for _, m := range nmc.Spec.Modules {
		if m.Config.Modprobe.ModuleName == "amdgpu" {
			return true
		}
	}
	return false
}
func (h *upgradeMgrHelper) unloadAMDGPUDriver(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) error {
	if h.isKMMManagedAMDGPU(ctx, networkConfig, node) {
		// if amdgpu related kernel modules are managed by KMM
		// KMM will auto unload them since amdgpu is not tolerating amd-network-driver-upgrade taint
		// no need to use worker pod to unload amdgpu
		return nil
	}
	// use script based worker pod to unload amdgpu driver
	// because amdgpu kmod may be inbox, pre-installed or KMM installed
	// using script based unload could work for all those cases
	logger := log.FromContext(ctx)
	if err := h.workerMgr.Work(ctx, networkConfig, node); err != nil {
		logger.Error(err, fmt.Sprintf("Node: %v. Failed to bring up worker pod to unload AMDGPU driver with error: %v", node.Name, err))
		return err
	}
	return nil
}

func (h *upgradeMgrHelper) loadAMDGPUDriver(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) error {
	logger := log.FromContext(ctx)

	// Check if amdgpu is managed by KMM or is inbox/pre-installed
	if h.isKMMManagedAMDGPU(ctx, networkConfig, node) {
		// Case 1: amdgpu is managed by KMM
		// if amdgpu related kernel modules are managed by KMM
		// uncordon the node will trigger KMM to load them back
		// no need to use worker pod to load amdgpu
		return nil
	}
	// Case 2: amdgpu is inbox/pre-installed - use worker pod to load it back
	if err := h.workerMgr.Undo(ctx, networkConfig, node); err != nil {
		logger.Error(err, fmt.Sprintf("Node: %v. Failed to bring up worker pod to load AMDGPU driver: %v", node.Name, err))
		return err
	}
	logger.Info(fmt.Sprintf("Node: %v. Triggered worker pod to load inbox/pre-installed AMDGPU driver", node.Name))
	// If the worker pod successfully loaded amdgpu
	// its node label would be removed by pod watcher automatically (internal/controllers/watchers/pod.go)
	// Wait for worker manager's node label to disappear (max 3 minutes)
	label := h.workerMgr.GetWorkReadyLabel(types.NamespacedName{Namespace: networkConfig.Namespace, Name: networkConfig.Name})
	for i := uint(0); i < 60; _, i = <-time.NewTicker(5*time.Second).C, i+1 {
		nodeObj := &v1.Node{}
		if err := h.client.Get(ctx, client.ObjectKey{Name: node.Name}, nodeObj); err == nil {
			if _, exists := nodeObj.Labels[label]; !exists {
				logger.Info(fmt.Sprintf("Node: %v. Worker pod node label successfully removed", node.Name))
				return nil
			}
		}
	}
	return fmt.Errorf("timed out waiting for worker pod node label to be removed on node %s", node.Name)
}

func (h *upgradeMgrHelper) removeWorkerPodNodeLabel(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) error {
	logger := log.FromContext(ctx)
	label := h.workerMgr.GetWorkReadyLabel(types.NamespacedName{Namespace: networkConfig.Namespace, Name: networkConfig.Name})
	if retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		nodeObj := &v1.Node{}
...
func (h *upgradeMgrHelper) patchWorkerPodIfNeeded(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) {
	logger := log.FromContext(ctx)
	// fetch latest NetworkConfig to reflect any spec changes
	latestNC := &amdv1alpha1.NetworkConfig{}
	if err := h.client.Get(ctx, types.NamespacedName{Namespace: networkConfig.Namespace, Name: networkConfig.Name}, latestNC); err == nil {
		networkConfig = latestNC
	}

	// derive expected image + secrets
	expectedUtilsImage := h.getWorkerPodImage(networkConfig)
	expectedImagePullSecrets := h.getWorkerImagePullSecrets(networkConfig)

	// get current worker pod (may create if missing)
	workerPod, err := h.workerMgr.GetWorkerPod(ctx, networkConfig, node)
	if k8serrors.IsNotFound(err) {
		if err := h.workerMgr.Work(ctx, networkConfig, node); err != nil {
			logger.Error(err, fmt.Sprintf("Node: %v. Failed to recreate worker pod with updated image/spec", node.Name))
		}
	} else if err != nil {
		logger.Error(err, fmt.Sprintf("Node: %v. Failed to get worker pod to check for its spec changes", node.Name))
		return
	}
	if workerPod != nil {
		if h.podImageOrPullSecretsChanged(ctx, workerPod, node, expectedImagePullSecrets, expectedUtilsImage) {
			if err := h.client.Delete(ctx, workerPod); err != nil {
				logger.Error(err, fmt.Sprintf("Node: %v. Failed to delete outdated worker pod %s/%s", node.Name, workerPod.Namespace, workerPod.Name))
			} else {
				logger.Info(fmt.Sprintf("Node: %v. Deleted outdated worker pod %s/%s; recreating", node.Name, workerPod.Namespace, workerPod.Name))
				// trigger recreation with updated spec
				if err := h.workerMgr.Work(ctx, networkConfig, node); err != nil {
					logger.Error(err, fmt.Sprintf("Node: %v. Failed to recreate worker pod with updated image/spec", node.Name))
				}
			}
...
func (h *upgradeMgrHelper) waitForAMDGPUUnload(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) error {
	logger := log.FromContext(ctx)

	if h.isKMMManagedAMDGPU(ctx, networkConfig, node) {
		// if amdgpu related kernel modules are managed by KMM
		// wait for KMM to auto unload them since amdgpu is not tolerating amd-network-driver-upgrade taint
		// when amdgpu driver is unloaded, it would be removed from NMC status modules list
		for i := uint(0); i < 60; _, i = <-time.NewTicker(5*time.Second).C, i+1 {
			nmc := &kmmv1beta1.NodeModulesConfig{}
			if err := h.client.Get(ctx, client.ObjectKey{Name: node.Name}, nmc); err == nil {
				amdgpuFound := false
				for _, m := range nmc.Status.Modules {
					if m.Config.Modprobe.ModuleName == "amdgpu" {
						amdgpuFound = true
						break
					}
				...
				if !amdgpuFound {
					logger.Info(fmt.Sprintf("Node: %v. KMM successfully unloaded AMDGPU driver", node.Name))
					return nil
				}
			}
			// if node became UpgradeComplete during wait, return
			if h.getNodeStatus(node.Name) == amdv1alpha1.UpgradeStateComplete || h.getNodeStatus(node.Name) == amdv1alpha1.UpgradeStateInstallComplete {
				logger.Info(fmt.Sprintf("Node: %v. Upgrade completed during AMDGPU unload wait, exiting wait", node.Name))
				return nil
			}
		}
		logger.Info(fmt.Sprintf("Node: %v. Timed out waiting for KMM to unload AMDGPU driver", node.Name))
		return fmt.Errorf("timed out waiting for KMM to unload AMDGPU driver on node %s", node.Name)
	}

	label := h.workerMgr.GetWorkReadyLabel(types.NamespacedName{Namespace: networkConfig.Namespace, Name: networkConfig.Name})
	for i := uint(0); i < 180; _, i = <-time.NewTicker(5*time.Second).C, i+1 {
		// check completion condition
		nodeObj := &v1.Node{}
		if err := h.client.Get(ctx, client.ObjectKey{Name: node.Name}, nodeObj); err == nil {
			if bootID, exists := nodeObj.Labels[label]; exists && bootID == nodeObj.Status.NodeInfo.BootID {
				logger.Info(fmt.Sprintf("Node: %v. AMDGPU unload worker pod completed successfully", node.Name))
				return nil
			}
		}
		// if node became UpgradeComplete during wait, return
		// possibly users modify the version back to original one
		if h.getNodeStatus(node.Name) == amdv1alpha1.UpgradeStateComplete || h.getNodeStatus(node.Name) == amdv1alpha1.UpgradeStateInstallComplete {
			logger.Info(fmt.Sprintf("Node: %v. Upgrade completed during AMDGPU unload wait, exiting wait", node.Name))
			return nil
		}
		h.patchWorkerPodIfNeeded(ctx, networkConfig, node)
	}

	logger.Info(fmt.Sprintf("Node: %v. Timed out waiting for AMDGPU unload worker pod to complete", node.Name))
	return fmt.Errorf("timed out waiting for AMDGPU unload worker pod to complete on node %s", node.Name)
}

func (h *upgradeMgrHelper) isNetworkConfigValid(ctx context.Context, nc *amdv1alpha1.NetworkConfig) bool {

	nwConfig := amdv1alpha1.NetworkConfig{}

	if err := h.client.Get(ctx, types.NamespacedName{Namespace: nc.Namespace, Name: nc.Name}, &nwConfig); err != nil {
		return false
	}

	if nc.Spec.Driver.Version != nwConfig.Spec.Driver.Version {
		return false
	}
...
func (h *upgradeMgrHelper) cordonOrUncordonNode(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node, cordon bool) error {
	...
	if err := h.addOrRemoveTaintToNode(ctx, networkConfig, node, cordon); err != nil {
...
func (h *upgradeMgrHelper) addOrRemoveTaintToNode(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node, taint bool) error {
...
func (h *upgradeMgrHelper) updateModuleVersionOnNode(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) error {
	...
	if retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		...
		nodeObjCopy := nodeObj.DeepCopy()
		driverVersion, err := utils.GetDriverVersion(*node, *networkConfig)
		if err == nil {
			nodeObj.Labels[fmt.Sprintf("kmm.node.kubernetes.io/version-module.%s.%s", networkConfig.Namespace, networkConfig.Name)] = driverVersion
		}
		return h.client.Patch(ctx, nodeObj, client.MergeFrom(nodeObjCopy))
...
func (h *upgradeMgrHelper) resetModuleVersionOnNode(ctx context.Context, networkConfig *amdv1alpha1.NetworkConfig, node *v1.Node) error {
	logger := log.FromContext(ctx)
	...
	if currentLabelVal, exists := nodeObj.Labels[labelKey]; exists {
		...
		if len(nmc.Status.Modules) > 0 {
			...
			for _, module := range nmc.Status.Modules {
				if module.Name == networkConfig.Name && module.Namespace == networkConfig.Namespace {
...
func (h *upgradeMgrHelper) handleNodeReboot(ctx context.Context, node *v1.Node, nc amdv1alpha1.NetworkConfig) {
	logger := log.FromContext(ctx)
	rebootPod := h.getRebootPod(node.Name, &nc)
	// Delete the existing pod if present
	pod := &v1.Pod{}
	if err := h.client.Get(ctx, types.NamespacedName{Namespace: nc.Namespace, Name: rebootPod.Name}, pod); err == nil {
		if err := h.client.Delete(ctx, pod); err != nil {
	...
	if h.shouldTriggerAMDGPUWorkerPod(ctx, &nc, node) {
		// unload AMDGPU driver before upgrading the network driver
		if err := h.unloadAMDGPUDriver(ctx, &nc, node); err != nil {
			logger.Error(err, fmt.Sprintf("Node: %v State: %v UpgradeFailed with Error: %v", node.Name, h.getNodeStatus(node.Name), err))
			// Mark the state as failed
			h.setNodeStatus(ctx, node.Name, amdv1alpha1.UpgradeStateFailed)
			return
	...
	if err := h.waitForAMDGPUUnload(ctx, &nc, node); err != nil {
		logger.Error(err, fmt.Sprintf("Node: %v State: %v UpgradeFailed with Error: %v", node.Name, h.getNodeStatus(node.Name), err))
		// Mark the state as failed
		h.setNodeStatus(ctx, node.Name, amdv1alpha1.UpgradeStateFailed)
		return
	...
	// Update expected module version on the node
	if err := h.updateModuleVersionOnNode(ctx, &nc, node); err != nil {
		logger.Error(err, fmt.Sprintf("Node: %v State: %v UpgradeFailed with Error: %v", node.Name, h.getNodeStatus(node.Name), err))
		// Mark the state as failed
		h.setNodeStatus(ctx, node.Name, amdv1alpha1.UpgradeStateFailed)
		return
	...
	waitForDriverUpgrade := func() {
		for i := uint(0); i < 360; _, i = <-time.NewTicker(10*time.Second).C, i+1 {
			...
			if err := h.client.Get(ctx, types.NamespacedName{Namespace: nc.Namespace, Name: node.Name}, nmcObj); err == nil {
				for _, status := range nmcObj.Status.Modules {
					if strings.HasSuffix(status.Config.ContainerImage, nc.Spec.Driver.Version) {
						return
					}
				}
			}
	...
	patchRebootPodIfNeeded := func() {
		// Fetch latest NetworkConfig to check if utils image changed
		fetchedNetworkConfig := &amdv1alpha1.NetworkConfig{}
		if err := h.client.Get(ctx, types.NamespacedName{Namespace: nc.Namespace, Name: nc.Name}, fetchedNetworkConfig); err == nil {
			expectedUtilsImage := h.getWorkerPodImage(fetchedNetworkConfig)
			expectedImagePullSecrets := h.getWorkerImagePullSecrets(fetchedNetworkConfig)

			// Check if reboot pod's image or imagePullSecrets differs from expected
			pod := &v1.Pod{}
			if err := h.client.Get(ctx, types.NamespacedName{Namespace: nc.Namespace, Name: rebootPod.Name}, pod); err == nil {
				if h.podImageOrPullSecretsChanged(ctx, pod, node, expectedImagePullSecrets, expectedUtilsImage) {
					rebootPod = h.getRebootPod(node.Name, fetchedNetworkConfig)
					// Delete the pod since pod specs are immutable
					if err := h.client.Delete(ctx, pod); err != nil {
						logger.Error(err, fmt.Sprintf("Node: %v Failed to delete reboot pod for recreation", node.Name))
					} else {
						logger.Info(fmt.Sprintf("Node: %v Deleted reboot pod for recreation with updated spec", node.Name))
					}
				}
			} else if k8serrors.IsNotFound(err) {
				_, err := controllerutil.CreateOrPatch(ctx, h.client, rebootPod, func() error {
					return nil
				})
				if err != nil {
					logger.Error(err, fmt.Sprintf("Node: %v State: %v RebootPod CreateOrPatch failed with Error: %v", node.Name, h.getNodeStatus(node.Name), err))
					// Mark the state as failed
					h.setNodeStatus(ctx, node.Name, amdv1alpha1.UpgradeStateRebootFailed)
					return
				}
			} else {
				logger.Error(err, fmt.Sprintf("Node: %v Failed to get reboot pod info", node.Name))
			}
		}
	}
	...
	fetchedNetworkConfig := &amdv1alpha1.NetworkConfig{}
	if err := h.client.Get(ctx, types.NamespacedName{Namespace: nc.Namespace, Name: nc.Name}, fetchedNetworkConfig); err != nil {
		logger.Error(err, "Failed to fetch NetworkConfig from API server")
		return
	}
	if fetchedNetworkConfig.Spec.Driver.Version == nc.Spec.Driver.Version {
		h.setNodeStatus(ctx, node.Name, amdv1alpha1.UpgradeStateRebootInProgress)
	}
	h.deleteRebootPod(ctx, node.Name, nc, false)

}

func (h *upgradeMgrHelper) deleteRebootPod(ctx context.Context, nodeName string, nc amdv1alpha1.NetworkConfig, force bool) {

	logger := log.FromContext(ctx)
	rebootPod := h.getRebootPod(nodeName, &nc)
	fetchedNetworkConfig := &amdv1alpha1.NetworkConfig{}
	pod := &v1.Pod{}
	if err := h.client.Get(ctx, types.NamespacedName{Namespace: nc.Namespace, Name: rebootPod.Name}, pod); err != nil {
		return
...
func (h *upgradeMgrHelper) getRebootPod(nodeName string, nc *amdv1alpha1.NetworkConfig) *v1.Pod {
	...
	if nc.Spec.CommonConfig.UtilsContainer.Image != "" {
		utilsImage = nc.Spec.CommonConfig.UtilsContainer.Image
	}
	imagePullSecrets := []v1.LocalObjectReference{}
	if nc.Spec.CommonConfig.UtilsContainer.ImageRegistrySecret != nil {
		imagePullSecrets = append(imagePullSecrets, *nc.Spec.CommonConfig.UtilsContainer.ImageRegistrySecret)
	}
	rebootPod := &v1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("amd-network-operator-%v-reboot-worker", nodeName),
	...
	if nc.Spec.CommonConfig.UtilsContainer.ImagePullPolicy != "" {
		rebootPod.Spec.Containers[0].ImagePullPolicy = v1.PullPolicy(nc.Spec.CommonConfig.UtilsContainer.ImagePullPolicy)
	}

	return rebootPod
}

func (h *upgradeMgrHelper) getWorkerPodImage(networkConfig *amdv1alpha1.NetworkConfig) string {
	image := defaultUtilsImage
	if h.isOpenShift {
		image = defaultOcUtilsImage
	}
	if networkConfig.Spec.CommonConfig.UtilsContainer.Image != "" {
		image = networkConfig.Spec.CommonConfig.UtilsContainer.Image
	}
	return image
}

func (h *upgradeMgrHelper) getWorkerImagePullSecrets(networkConfig *amdv1alpha1.NetworkConfig) []v1.LocalObjectReference {
	imagePullSecrets := []v1.LocalObjectReference{}
	if networkConfig.Spec.CommonConfig.UtilsContainer.ImageRegistrySecret != nil {
		imagePullSecrets = append(imagePullSecrets, *networkConfig.Spec.CommonConfig.UtilsContainer.ImageRegistrySecret)
	}
	return imagePullSecrets
}

func (h *upgradeMgrHelper) podImageOrPullSecretsChanged(ctx context.Context, pod *v1.Pod, node *v1.Node,
	expectedImagePullSecrets []v1.LocalObjectReference, expectedUtilsImage string) bool {
	logger := log.FromContext(ctx)
	needsPatch := false
	// Compare image
	if len(pod.Spec.Containers) > 0 && pod.Spec.Containers[0].Image != expectedUtilsImage {
		logger.Info(fmt.Sprintf("Node: %v Reboot pod image changed from %v to %v, patching pod", node.Name, pod.Spec.Containers[0].Image, expectedUtilsImage))
		pod.Spec.Containers[0].Image = expectedUtilsImage
		needsPatch = true
	}
	// Compare imagePullSecrets
	secretsChanged := len(pod.Spec.ImagePullSecrets) != len(expectedImagePullSecrets)
	if !secretsChanged {
		for i := range expectedImagePullSecrets {
			if i >= len(pod.Spec.ImagePullSecrets) || pod.Spec.ImagePullSecrets[i].Name != expectedImagePullSecrets[i].Name {
				secretsChanged = true
				break
			}
		}
	...
	if secretsChanged {
		logger.Info(fmt.Sprintf("Node: %v Reboot pod imagePullSecrets changed, patching pod", node.Name))
		pod.Spec.ImagePullSecrets = expectedImagePullSecrets
		needsPatch = true
	}
	return needsPatch
}
