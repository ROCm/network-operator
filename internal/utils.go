...
import (
	...
	"github.com/ROCm/network-operator/internal/cmd"
)
...
const (
	...
	WorkAction             = "work"
	UndoAction             = "undo"
	WorkerActionLabelKey   = "network.operator.amd.com/worker-action"
	WorkReadyLabelTemplate = "network.operator.amd.com/%v.%v.work.ready"
)

func GetDriverVersion(node v1.Node, networkConfig amdv1alpha1.NetworkConfig) (string, error) {
	var driverVersion string
	var err error
	if networkConfig.Spec.Driver.Version != "" {
		driverVersion = networkConfig.Spec.Driver.Version
	} else {
...
func GetUpgradeWorkerPodName(networkConfig *amdv1alpha1.NetworkConfig, nodeName string) string {
	return fmt.Sprintf("worker-%v-%v", networkConfig.Name, nodeName)
}
...
// IsPrometheusServiceMonitorEnable checks if the Prometheus ServiceMonitor is enabled in the NetworkConfig
func IsPrometheusServiceMonitorEnable(nwConfig *amdv1alpha1.NetworkConfig) bool {
	if nwConfig.Spec.MetricsExporter.Prometheus != nil &&
		nwConfig.Spec.MetricsExporter.Prometheus.ServiceMonitor != nil &&
		nwConfig.Spec.MetricsExporter.Prometheus.ServiceMonitor.Enable != nil &&
		*nwConfig.Spec.MetricsExporter.Prometheus.ServiceMonitor.Enable {
		return true
	}
	return false
}
