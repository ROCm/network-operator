/*
Copyright (c) Advanced Micro Devices, Inc. All rights reserved.

Licensed under the Apache License, Version 2.0 (the \"License\");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

     http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an \"AS IS\" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

package e2e

import (
	"context"
	"flag"
	"fmt"
	"os"
	"path"
	"path/filepath"
	"runtime"
	"sort"
	"strings"
	"time"

	netclientset "github.com/k8snetworkplumbingwg/network-attachment-definition-client/pkg/client/clientset/versioned"
	monitoringClient "github.com/prometheus-operator/prometheus-operator/pkg/client/versioned"
	"github.com/sirupsen/logrus"
	"github.com/stretchr/testify/assert"
	. "gopkg.in/check.v1"
	apiextv1 "k8s.io/apiextensions-apiserver/pkg/apis/apiextensions/v1"
	apiextClient "k8s.io/apiextensions-apiserver/pkg/client/clientset/clientset"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"k8s.io/client-go/kubernetes"
	"k8s.io/client-go/kubernetes/scheme"
	"k8s.io/client-go/tools/clientcmd"
	"k8s.io/client-go/util/homedir"

	"github.com/ROCm/network-operator/tests/e2e/client"
	"github.com/ROCm/network-operator/tests/e2e/utils"
)

const (
	timeoutShort = 5 * time.Minute
	timeoutLong  = 45 * time.Minute
	pollInterval = 5 * time.Second
)

var (
	kubeConfig    = flag.String("kubeConfig", filepath.Join(homedir.HomeDir(), ".kube", "config"), "kubeconfig path")
	helmChart     = flag.String("helmchart", "", "helm chart reference")
	operatorNS    = flag.String("namespace", "kube-amd-network", "operator namespace")
	cfgName       = flag.String("networkConfigName", "networkconfig-example", "NetworkConfig name")
	driverVersion = flag.String("driverVersion", "1.117.1-a-42", "driver version")
	openshift     = flag.Bool("openshift", false, "openshift deployment")
	simEnable     = flag.Bool("simEnable", false, "simulate (no hardware)")
	ciEnv         = flag.Bool("ciEnv", false, "CI environment")
)

var logger = logrus.Logger{
	Out: os.Stdout,
	Formatter: &logrus.TextFormatter{
		CallerPrettyfier: func(f *runtime.Frame) (string, string) {
			return fmt.Sprintf("%v()", f.Function), fmt.Sprintf("%v:%v", path.Base(f.File), f.Line)
		},
	},
	Hooks:        make(logrus.LevelHooks),
	Level:        logrus.InfoLevel,
	ExitFunc:     os.Exit,
	ReportCaller: true,
}

// E2ESuite e2e config
type E2ESuite struct {
	k8sClientSet         *kubernetes.Clientset
	nadClient            *netclientset.Clientset
	nCfgClient           *client.NetworkConfigClient
	apiClientSet         *apiextClient.Clientset
	cfgName              string
	helmChart            string
	ns                   string
	kubeconfig           string
	clusterType          string
	defaultDriverVersion string
	openshift            bool
	simEnable            bool
	ciEnv                bool
	monClient            monitoringClient.Interface
}

func (s *E2ESuite) SetUpSuite(c *C) {
	flag.Parse()
	logger.Infof("SetUpSuite start")

	s.helmChart = *helmChart
	s.kubeconfig = *kubeConfig
	s.ns = *operatorNS
	s.cfgName = *cfgName
	s.defaultDriverVersion = *driverVersion
	s.openshift = *openshift
	s.simEnable = *simEnable
	s.ciEnv = *ciEnv

	// use the current context in kubeconfig
	config, err := clientcmd.BuildConfigFromFlags("", s.kubeconfig)
	if err != nil {
		c.Fatalf("Error: %v", err.Error())
	}

	s.nCfgClient, err = client.Client(config)
	if err != nil {
		c.Fatalf("Error: %v", err.Error())
	}

	err = apiextv1.AddToScheme(scheme.Scheme)
	if err != nil {
		c.Fatalf("Error: %v", err.Error())
	}

	// creates the clientset
	s.k8sClientSet, err = kubernetes.NewForConfig(config)
	if err != nil {
		c.Fatalf("Error: %v", err.Error())
	}

	// create the apiext clientset
	s.apiClientSet, err = apiextClient.NewForConfig(config)
	if err != nil {
		c.Fatalf("Error: %v", err.Error())
	}

	// create the monitoring client
	monClient, err := monitoringClient.NewForConfig(config)
	if err != nil {
		c.Fatalf("Error: %v", err.Error())
	}
	s.monClient = monClient

	// Create NAD client
	s.nadClient, err = netclientset.NewForConfig(config)
	if err != nil {
		c.Fatalf("failed to create NAD client: %v", err)
	}

	s.clusterType = utils.GetClusterType(config)

	if s.openshift {
		assert.Eventually(c, func() bool {
			if err := utils.CheckHelmDeployment(s.k8sClientSet, s.ns, true); err != nil {
				logger.Infof("%v", err)
				return false
			}
			return true
		}, 5*time.Minute, 5*time.Second)
	}
	if s.ciEnv {
		if err := utils.PatchKMMDeploymentWithCIENVFlag(s.k8sClientSet); err != nil {
			c.Fatalf("%v", err)
		}
		if err := utils.PatchOperatorControllerDeploymentWithCIENVFlag(s.k8sClientSet); err != nil {
			c.Fatalf("%v", err)
		}
	}
	logger.Infof("SetUpSuite complete")
}

func (s *E2ESuite) SetUpTest(c *C) {
	logger.Info("SetUpTest start")
	_ = s.k8sClientSet.CoreV1().ConfigMaps(s.ns).Delete(context.TODO(), s.cfgName, metav1.DeleteOptions{})
	logger.Info("SetUpTest complete")
}

func (s *E2ESuite) TearDownTest(c *C) {
	logger.Info("TearDownTest start")
	s.deleteAllNetworkConfigs(c)
	s.deleteConfigMap()
	logger.Info("TearDownTest complete")
}

func (s *E2ESuite) TearDownSuite(c *C) {
	logger.Info("TearDownSuite start")
	s.deleteAllNetworkConfigs(c)
	s.deleteConfigMap()
	logger.Info("TearDownSuite complete")
}

func boolPtr(b bool) *bool { return &b }

func waitEventually(c *C, desc string, timeout, interval time.Duration, fn func() (bool, error)) {
	assert.Eventually(c, func() bool {
		ok, err := fn()
		if err != nil {
			logger.Infof("%s transient error: %v", desc, err)
			return false
		}
		return ok
	}, timeout, interval, desc)
}

func selectorString(m map[string]string) string {
	keys := make([]string, 0, len(m))
	for k := range m {
		keys = append(keys, k)
	}
	sort.Strings(keys)
	out := make([]string, 0, len(keys))
	for _, k := range keys {
		out = append(out, fmt.Sprintf("%s=%s", k, m[k]))
	}
	return strings.Join(out, ",")
}
